...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost_distribution in livingGhostPositionDistributions:
            temp_ghost_pos = ghost_distribution.argMax()
            temp_distance = self.distancer.getDistance(pos1=temp_ghost_pos, pos2=pacmanPosition)
            ## define which ghost is closet
            if temp_distance < lowest_distance:
                lowest_distance = temp_distance
                ghostPosition = temp_ghost_pos
        
        ## choose action (1 move) that minimize the maze distance to the closet ghost
        lowest_dist = float("inf")
        action = None
        for legal_action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, legal_action)
            temp_dist = self.distancer.getDistance(successorPosition, ghostPosition)
