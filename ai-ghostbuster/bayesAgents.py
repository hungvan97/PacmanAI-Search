...
# the observations are measurment that Pacman makes while traveling around the board
BLUE_OBS_VAL = "blue"           
RED_OBS_VAL = "red"             
NO_OBS_VAL = "none"             # no house
...
def constructBayesNet(gameState):
    ...
    # 2. populate `edges` with every edge in the Bayes Net (a tuple `(from, to)`)
    # Step 2 from layer 1 to layer 2
    ...
    # 3. set each `variableDomainsDict[var] = values`, where `values` is the set
    # of possible assignments to `var`. These should again be set using the
    # constants defined at the top of this file
    variableDomainsDict = {}
    ...
    for obsVar in obsVars:
        # Step 2 from layer 2 to layer 3
        edges.append((FOOD_HOUSE_VAR, obsVar))
        edges.append((GHOST_HOUSE_VAR, obsVar))
        # Step 3 for new edge between layer 2 to layer 3
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment in query_factor.getAllPossibleAssignmentDicts():
        temp_probability = query_factor.getProbability(assignment)
        if prob < temp_probability:
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        for assignmentDict in query_factor.getAllPossibleAssignmentDicts():
            if assignmentDict[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assignmentDict[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                p_food_left = query_factor.getProbability(assignmentDict)
            if assignmentDict[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignmentDict[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                p_food_right = query_factor.getProbability(assignmentDict)

        EU_left = p_food_left * WON_GAME_REWARD + (1 - p_food_left) * GHOST_COLLISION_REWARD
        EU_right = p_food_right * WON_GAME_REWARD + (1 - p_food_right) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            ...
            observedEvidence.update(explorationEvidence)
            left, right = self.computeEnterValues(observedEvidence, enterEliminationOrder)
            expectedValue += max(left, right)*prob  
        "*** END YOUR CODE HERE ***"
