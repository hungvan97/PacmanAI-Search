...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # iterating over all the hidden variable
        # (perform inference by interleaving joining on a variable
        # and eliminating that variable, in the order of variables according
        # to eliminationOrder.)
        for hidden_var in eliminationOrder:
            # interleave join
            currentFactorsList, newFactor = joinFactorsByVariable(currentFactorsList, hidden_var)
            # ignore special case where joined Factore only has 1 unconditioned variable
            if len(newFactor.unconditionedVariables()) > 1:
                # and eliminate
                factor_after_elim = eliminate(newFactor, hidden_var)
                currentFactorsList.append(factor_after_elim)
        finalFactor = joinFactors(currentFactorsList)
        # The sum of the probabilities should sum to one ==> normalize
